...
function installKeyboardShield(rootEl) {
  ...
  const Original = {
    ...
    doc_onkeypress: document.onkeypress,
    doc_onkeyup: document.onkeyup,
    win_onkeydown: window.onkeydown,
    win_onkeypress: window.onkeypress,
    win_onkeyup: window.onkeyup
  };
  ...
  function wrappedAdd(type, listener, options) {
    if (!isKeyboardType(type) || typeof listener !== 'function') {
      return Original.addEventListener.call(this, type, listener, options);
    }
    if (wrapMap.has(listener)) {
      const existing = wrapMap.get(listener);
      return Original.addEventListener.call(this, type, existing, options);
    }
    ...
    wrapMap.set(listener, wrapper);
    return Original.addEventListener.call(this, type, wrapper, options);
  }
  function wrappedRemove(type, listener, options) {
    if (!isKeyboardType(type) || typeof listener !== 'function') {
      return Original.removeEventListener.call(this, type, listener, options);
    }
    const wrapper = wrapMap.get(listener);
    if (wrapper) {
      Original.removeEventListener.call(this, type, wrapper, options);
      wrapMap.delete(listener);
    } else {
      Original.removeEventListener.call(this, type, listener, options);
    }
  }
  EventTarget.prototype.addEventListener = wrappedAdd;
  EventTarget.prototype.removeEventListener = wrappedRemove;
  function makePropWrapper(orig) {
    return function (e) {
      try {
        if (focusInsideBox()) {
          return;
      ...
      if (typeof orig === 'function') {
        try { return orig.call(this, e); } catch (err) { console.error(err); }
      }
    };
  }
  document.onkeydown = makePropWrapper(Original.doc_onkeydown);
  document.onkeypress = makePropWrapper(Original.doc_onkeypress);
  document.onkeyup = makePropWrapper(Original.doc_onkeyup);
  window.onkeydown = makePropWrapper(Original.win_onkeydown);
  window.onkeypress = makePropWrapper(Original.win_onkeypress);
  window.onkeyup = makePropWrapper(Original.win_onkeyup);
  function uninstall() {
    ...
    document.onkeydown = Original.doc_onkeydown;
    document.onkeypress = Original.doc_onkeypress;
    document.onkeyup = Original.doc_onkeyup;
    window.onkeydown = Original.win_onkeydown;
    window.onkeypress = Original.win_onkeypress;
